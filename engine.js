...
async function syncWork(source, sourceId, opts = {}) {
  ...
  let enriched = {};
  // enrichment only when requested — bulk syncs skip it to respect AniList rate limits
  if (source === 'mangadex' && opts.enrich !== false) {
    try {
  ...
  if (opts.deep !== false) {
    ...
    for (const ch of chapters) {
      ...
      if (!exists) {
        ...
        added++;
      } else if (exists.work_id !== saved.id) {
        // orphan from a previous import — re-point it to the current work
        db.update('chapters', exists.id, { ...ch, work_id: saved.id, synced_at: new Date().toISOString() });
        added++;
...
async function syncMany(quota = 1000, opts = {}, onProgress) {
  ...
  for (const m of list) {
    try {
      const w = await syncWork('mangadex', m.id, { deep, lang, enrich: false });
...
async function deepAll(opts = {}, onProgress) {
  ...
  for (let i = 0; i < existing.length; i++) {
    const w = existing[i];
    try {
      await syncWork(w.source, w.source_id, { deep: true, lang, enrich: false });
      syncedExisting++;
    } catch {}
